...
fama.data$Hi <- 0.2*( fd$SMALL.HiBM + fd$ME2.BM5 + fd$ME3.BM5 + fd$ME4.BM5 + fd$BIG.HiBM )
fama.data$Lo <- 0.2*( fd$SMALL.LoBM + fd$ME2.BM1 + fd$ME3.BM1 + fd$ME4.BM1 + fd$BIG.LoBM )
...
ts.data <- data.frame(fama.data$Hi)
row.names(ts.data) <- date.seq

z <- apply.rolling(ts.data, width=width, FUN = "Return.custom")
fama.data$Hi.5YrRolling <- z[[1]]

ts.data <- data.frame(fama.data$Lo)
row.names(ts.data) <- date.seq

z <- apply.rolling(ts.data, width=width, FUN = "Return.custom")
fama.data$Lo.5YrRolling <- z[[1]]

fama.data$Hi.Lo.5YrRolling <- fama.data$Hi.5YrRolling - fama.data$Lo.5YrRolling 
...
for (i in 3:length(start.indexes)) {
    ...
    for (j in start.idx:end.idx) {
        ...
        hi.val <- (hi.val+1)*(fama.data$Hi[j]+1)-1
        lo.val <- (lo.val+1)*(fama.data$Lo[j]+1)-1
        cur.value <- hi.val - lo.val
