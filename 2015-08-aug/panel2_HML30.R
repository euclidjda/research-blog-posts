...
# http://mba.tuck.dartmouth.edu/pages/faculty/ken.french/ftp/6_Portfolios_2x3_CSV.zip
...
fama.data$Hi.Lo <- 0.5*(fama.data$BIG.HiBM + fama.data$SMALL.HiBM) - 0.5*(fama.data$BIG.LoBM + fama.data$SMALL.LoBM)
...
colors <- rev( brewer.pal(7,"Blues") )[1:6]
...
g <- g + geom_line()
g <- g + scale_size_manual(values=c(1,1,1,1,1,1,2))
